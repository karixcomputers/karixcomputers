...
export default function TicketDetail() {
  ...
  // Folosim variabila de mediu pentru API sau domeniul oficial
  const API_URL = import.meta.env.VITE_API_URL || "https://api.karixcomputers.ro";
              ...
              return (
                <div key={msg.id} className={`flex ${isAdminMsg ? 'justify-start' : 'justify-end'} animate-in slide-in-from-bottom-2 duration-300`}>
                  <div className={`max-w-[75%] p-5 rounded-[25px] shadow-2xl border ${
                    ...
                    {msg.text && <p className="text-sm font-medium leading-relaxed whitespace-pre-wrap text-left">{msg.text}</p>}
                  </div>
                </div>
              );
            })}
