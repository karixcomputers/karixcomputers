...
export default function Tickets() {
  ...
  return (
    <>
      {/* SEO: CONFIGURARE PENTRU CENTRUL DE SUPORT */}
      <SEO 
        title="Tichete Suport" 
        description="Gestionează solicitările tale de asistență tehnică. Deschide un tichet nou pentru probleme hardware, software sau întrebări despre garanție."
      />
      ...
      <div className="min-h-screen pt-32 pb-24 px-4 relative overflow-hidden bg-transparent text-left">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          <header className="mb-12 flex flex-col md:flex-row justify-between items-end gap-6 text-left">
            <div>
              <h1 className="text-5xl font-black text-white tracking-tighter mb-2 italic drop-shadow-2xl uppercase">
          ...
          <div className="space-y-6">
            {isLoading ? (
              ...
              <div className="p-20 rounded-[45px] bg-white/5 border border-white/10 backdrop-blur-xl text-center shadow-2xl border-dashed">
                <div className="text-5xl mb-6 opacity-20 italic font-black uppercase">Ticket Empty</div>
              ...
              tickets.map((t) => (
                <Link key={t.id} to={`/tickets/${t.id}`} className="group block p-[1px] rounded-[35px] bg-gradient-to-br from-white/10 to-transparent hover:from-indigo-500/30 transition-all duration-500">
                  <div className="p-8 rounded-[34px] bg-[#0b1020]/70 backdrop-blur-3xl flex flex-col md:flex-row md:items-center justify-between gap-6 text-left">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
            <div className="bg-[#0f172a] border border-white/10 p-8 md:p-10 rounded-[40px] max-w-lg w-full shadow-2xl animate-in zoom-in duration-300 relative overflow-hidden">
              ...
              <h2 className="text-3xl font-black text-white uppercase italic mb-2 relative z-10 text-left">Tichet Nou</h2>
              <p className="text-gray-500 text-[10px] uppercase font-black tracking-widest mb-8 relative z-10 text-left">Explică-ne cu ce te putem ajuta</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-left relative z-10">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-indigo-500 transition-all font-medium"
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#111827] border border-white/10 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-indigo-500 transition-all cursor-pointer font-medium italic"
                  >
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-indigo-500 transition-all resize-none font-medium italic"
                    placeholder="Descrie problema ta aici..."
                  ></textarea>
                </div>
                <div className="flex gap-4 mt-10">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-4 text-gray-500 font-black uppercase text-[10px] hover:text-white transition-colors tracking-widest italic">Anulare</button>
                  <button 
                    type="submit" 
                    disabled={createTicketMutation.isPending}
                    className="flex-1 py-4 rounded-2xl bg-indigo-600 text-white font-black uppercase text-[10px] shadow-xl hover:bg-indigo-500 transition-all italic active:scale-95 disabled:opacity-50 tracking-widest"
                  >
