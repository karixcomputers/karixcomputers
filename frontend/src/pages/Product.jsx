...
export default function Product() {
  ...
  const inStock = p.priceCents > 0;

  return (
    <>
      {/* SEO DINAMIC: Titlu și descriere generate automat din datele produsului */}
      <SEO 
        title={p.name} 
        description={`${p.name} - ${p.description?.substring(0, 150) || "Sistem PC de înaltă performanță configurat și testat de echipa Karix Computers."} ... Comandă acum cu livrare rapidă și garanție.`}
        image={p.images?.[0]}
        url={`https://karixcomputers.ro/product/${p.id}`}
      />
      ...
      <div className="min-h-screen pt-32 pb-24 px-4 bg-[#0b1020] text-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-xs font-black uppercase tracking-widest text-gray-500 mb-8 flex gap-2">
            ...
            <span>/</span> 
            <span className="text-indigo-400 italic text-left">{p.name}</span>
          </div>
